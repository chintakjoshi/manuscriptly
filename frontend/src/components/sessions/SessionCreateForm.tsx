...
export function SessionCreateForm({ userId, onUserIdChange, onCreate, loading = false }: SessionCreateFormProps) {
  ...
  return (
    <form onSubmit={(event) => void handleSubmit(event)} className="space-y-2 rounded-xl border border-slate-200 bg-white p-3">
      ...
      <label className="block text-xs text-slate-600" htmlFor="session-user-id">
        User ID
      </label>
      <input
        id="session-user-id"
